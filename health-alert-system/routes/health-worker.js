...
module.exports = (db, upload, io) => {
    ...
    router.post('/register', async (req, res) => {
        const { 
            username, email, password, full_name, phone, lga,
            workplace_type, workplace_name, workplace_lga, workplace_address,
        ...
        try {
            ...
            db.run(
                `INSERT INTO users (username, email, password, role, full_name, phone, lga, 
                 workplace_type, workplace_name, workplace_lga, workplace_address,
                 security_question1, security_answer1, security_question2, security_answer2) 
                 VALUES (?, ?, ?, 'health_worker', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                [username, email, hashedPassword, full_name, phone, lga,
                 workplace_type, workplace_name, workplace_lga, workplace_address,
                 security_question1, hashedAnswer1, security_question2, hashedAnswer2],
                function(err) {
                    if (err) {
    ...
    router.post('/login', async (req, res) => {
        ...
        try {
            const user = await new Promise((resolve, reject) => {
                db.get("SELECT * FROM users WHERE username = ? AND role = 'health_worker'", [username], (err, row) => {
                    if (err) reject(err);
                    else resolve(row);
                });
            });
            ...
            req.session.user = {
                ...
                workplace_lga: user.workplace_lga
            };
    ...
    // Get health worker dashboard data
    router.get('/dashboard', (req, res) => {
        if (!req.session.user || req.session.user.role !== 'health_worker') {
...
db.all("SELECT id FROM users WHERE role = 'admin'", [], (err, admins) => {
    ...
    router.post('/profile/update', upload.single('profile_photo'), (req, res) => {
        ...
        const { full_name, email, phone, lga, workplace_type, workplace_name, workplace_lga, workplace_address } = req.body;
        const profile_photo = req.file ? `/uploads/${req.file.filename}` : undefined;

        let query, params;

        if (profile_photo) {
            query = `UPDATE users SET full_name = ?, email = ?, phone = ?, lga = ?, 
                    workplace_type = ?, workplace_name = ?, workplace_lga = ?, workplace_address = ?, profile_photo = ? 
                    WHERE id = ?`;
            params = [full_name, email, phone, lga, workplace_type, workplace_name, workplace_lga, workplace_address, profile_photo, req.session.user.id];
        } else {
            query = `UPDATE users SET full_name = ?, email = ?, phone = ?, lga = ?, 
                    workplace_type = ?, workplace_name = ?, workplace_lga = ?, workplace_address = ? 
                    WHERE id = ?`;
            params = [full_name, email, phone, lga, workplace_type, workplace_name, workplace_lga, workplace_address, req.session.user.id];
        }
        ...
        db.run(query, params, function(err) {
            ...
            req.session.user.workplace_type = workplace_type;
            req.session.user.workplace_name = workplace_name;
            req.session.user.workplace_lga = workplace_lga;
    ...
    router.post('/forgot-password/verify', async (req, res) => {
        ...
        try {
            ...
            // Store temporary reset token in session
    ...
    // Add these routes to your existing health-worker.js file
...
router.post('/forgot-password', (req, res) => {
    ...
    const query = `
        ...
        FROM users 
        WHERE (email = ? OR username = ?) AND role = 'health_worker' AND is_verified = 1
    `;
...
router.post('/verify-security-answers', (req, res) => {
    ...
    const query = `
        ...
        FROM users 
        WHERE id = ? AND role = 'health_worker'
    `;
...
router.post('/reset-password', (req, res) => {
    ...
    bcrypt.hash(newPassword, 10, (err, hashedPassword) => {
        ...
        const query = `UPDATE users SET password = ? WHERE id = ? AND role = 'health_worker'`;
